...
def login_screen():
    ...
    # Add the logo
    try:
        img = PhotoImage(file='log.png')  # Ensure the `log.png` file is in the correct location
        logo_label = customtkinter.CTkLabel(left_frame, image=img, text="", fg_color="white")
        logo_label.pack(expand=True)  # Center the image vertically
    except Exception as e:
        # Show a warning if the logo file is missing or invalid
        messagebox.showwarning("Warning", f"Logo not found or invalid: {str(e)}")
